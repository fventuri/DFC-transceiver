...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "c:s:q:t:w:m:b:n:o:")) != -1) {
        ...
        case 'n':
            ...
            break;
        case 'o':
            outputOstream.open(optarg, std::ios_base::out | std::ios_base::binary);
            if (!outputOstream) {
    ...
    while (1) {
        ...
        //outputOstream.write((const UINT8 *)buffer, length);
        outputOstream.write((char *) buffer, length);
        if (!outputOstream) {
